...
Working classification for [ADR 14](../architecture/decisions/0014-ai-assists-staff-read-and-draft-only.md), 2026-09-19. It is an architect's reading of Regulation (EU) 2024/1689 (the AI Act), not legal advice. Counsel confirms it together with the DPIA (plan step S100) before any use case goes live.

## Roles

Hopin is the **deployer** of each system and does not place a model on the market. Anthropic is the provider of the general-purpose model, and AWS operates the Bedrock service ([ADR 15](../architecture/decisions/0015-bedrock-for-data-openrouter-for-evaluation.md)). Partners are deployers too when their staff use the console.
...
No use case interacts with passengers or callers directly, so the Article 50 duty to disclose a chatbot does not arise. A voicebot for phone orders would change that, and it is out of scope.

## Boundaries that keep this classification

- **No AI in matching, pricing, driver scoring, account blocking or refunds.** These allocate or evaluate work, or decide about money ([ADR 14](../architecture/decisions/0014-ai-assists-staff-read-and-draft-only.md)).
- **The meter-receipt check produces a number, not a verdict.** The partner sees the photo, the typed amount and the read amount, and decides. Its output is never stored against the driver as a score.
- **The Platform Work Directive applies anyway** to any automated monitoring of drivers. Hopin's AI does not monitor drivers, and this must stay true in the DPIA.
...
Any new use case, any use that touches a driver's work, or any tool that lets the model act instead of draft.
