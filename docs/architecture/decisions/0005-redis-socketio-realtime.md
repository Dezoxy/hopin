...
Online drivers report a position every few seconds. Matching needs the nearest online drivers within milliseconds. Passengers must see the driver move in near real time, across several API tasks.
...
We propose keeping live driver positions only in Redis, using Socket.IO with the Redis adapter for fan-out, and BullMQ on the same Redis for timers and jobs. PostgreSQL receives batched location history for disputes.
