...
| QA-02 | Location freshness | Driver position ≤ 3 s old on the passenger screen; ≤ 5 s at ≤ 20 m from the car device | Socket.IO fan-out through Redis; 3 s emit interval ([C-05](constraints.md)) | Load test (S092). Local slice: p95 2.85 s, max 3.01 s; the 3 s report interval leaves no headroom ([evidence](../evidence/s123-slice-results.md)) |
