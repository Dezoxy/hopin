...
Azure: one tenant and one subscription, `hopin-secondary`, with resource groups
`rg-hopin-backup` and `rg-hopin-vault`. Region to be chosen in plan step S012
(West Europe or Germany West Central).

Service control policies deny regions outside the EU and use of the root user in
every account (plan step S011).
