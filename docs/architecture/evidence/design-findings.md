...
| 4 | The first payment design hand-coded retries, waits and callbacks in a job queue that one operator would have to own. | Review of the ADR 12 draft | A managed Step Functions workflow per ride, named by the ride ID, replaced the queue. Stripe idempotency keys still prevent double charges, and the nightly reconciliation stays. | [ADR 12](../decisions/0012-payment-capture-workflow.md) |
| 5 | Road-ETA matching makes Mapbox almost five times the AWS bill by year 3 of the ambitious case. | Three-year cost model (S120) | Pre-ranking by straight-line distance and sending only the best three to the routing API became the first cost lever, triggered when Mapbox spend passes AWS spend. Self-hosted routing is the second. | [RISK-019](../risks/architecture-risks.md), [cost model](../../business/three-year-cost-model.md) |
