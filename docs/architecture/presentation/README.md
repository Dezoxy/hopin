...
Communication diagrams are accepted views redrawn for one audience, following
architecture-base's `presentation.md`. The Structurizr model stays the truth. A
row here says which view and model commit a redraw was checked against. A row
goes stale when its source view changes; re-check it by hand and re-date it, or
remove it.
...
Each Mermaid block carries an HTML comment naming its source view and model
commit. Mermaid lays the diagrams out itself, so they are checked for content,
not geometry. For the checked layout, see the architecture PDF.

Sequence-diagram messages must not contain semicolons: Mermaid treats one as the
end of a statement and fails to parse the rest.
