...
| `/passenger` | `driver.position` | Server → passenger | ride ID, lat, lng, heading | Every 2.5 s | [QA-02](../requirements/quality-attributes.md) |
| `/passenger` | `ride.eta` | Server → passenger | ride ID, ETA seconds | On change | |
| `/driver` | `location` | Driver → server | lat, lng, heading, speed, accuracy | Every 2.5 s while online | ≤ 5 s at ≤ 20 m required by [C-05](../requirements/constraints.md) |
